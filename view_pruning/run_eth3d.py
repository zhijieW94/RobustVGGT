#!/usr/bin/env python3
"""Batch-run a robust view-pruning method over every sequence in the eth3d dataset.

Dataset layout (example):
    <dataset_root>/<seq_name>/images/dslr_images/*.JPG

Only sequences with a non-empty `images/dslr_images/` subfolder are processed.
...
def find_image_dir(seq_dir: Path) -> Optional[Path]:
    """Locate the `images/dslr_images/` folder inside an eth3d sequence dir."""
    candidate = seq_dir / "images" / "dslr_images"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a robust view-pruning method on every eth3d sequence."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Root directory of the eth3d dataset (default: %(default)s).",
    )
    parser.add_argument(
