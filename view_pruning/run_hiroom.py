#!/usr/bin/env python3
"""Batch-run a robust view-pruning method over every sequence in the hiroom dataset.

Dataset layout (example):
    <dataset_root>/<seq_idx>/<cam_sampled_XX>/image/*.jpg

The `cam_sampled_XX` folder name can vary per sequence, so it is discovered
dynamically. Only sequences with a non-empty `image/` subfolder are processed.
...
def find_image_dir(seq_dir: Path) -> Optional[Path]:
    """Locate the `image/` folder inside a sequence dir.

    Walks down `cam_sampled_*` children (name may differ per sequence) and
    returns the first one that contains an `image/` subfolder with images.
    Falls back to any immediate `image/` child.
    """
    direct = seq_dir / "image"
    if direct.is_dir() and any(p.suffix in VALID_IMAGE_SUFFIXES for p in direct.iterdir()):
        return direct
...
def discover_sequences(dataset_root: Path) -> List[Tuple[str, Path]]:
    """Return (seq_idx, image_dir) pairs sorted by sequence name."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a robust view-pruning method on every hiroom sequence."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Root directory of the hiroom dataset (default: %(default)s).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Restrict to these sequence indices (space-separated). Default: all sequences.",
    )
    parser.add_argument(
